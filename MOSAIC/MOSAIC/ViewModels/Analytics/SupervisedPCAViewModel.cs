...
/// <summary>
/// ViewModel for the Supervised PCA visualisation card.
/// </summary>
/// <remarks>
/// <para>
/// Bridges the <see cref="SupervisedPCA"/> model and the Avalonia UI. It subscribes to the model
...
/// <see cref="_liveScatterPointsUI"/> shows the continuous real-time PC1/PC2 projection.
...
/// <see cref="ScatterSeries{TModel}"/> instances. The entire series array is rebuilt by
/// <see cref="RebuildAllSeriesUi"/> whenever <see cref="_clusterUpdatePending"/> is set.
/// During an active capture session the rebuild is throttled to once per 500 ms to prevent
/// chart jitter.
/// </para>
/// <para>
/// <b>3D scatter:</b> When the model runs in 3-component mode (<see cref="Is3D"/> is
/// <see langword="true"/>), a <see cref="Scatter3DMonitor"/> is created at construction time.
/// Live points are streamed with class index 0; captured cluster points are frozen into the
/// monitor by <see cref="RebuildFrozen3DPoints"/> and coloured per class using
/// <see cref="Scatter3DMonitor.SetClassColor"/>. Class indices are assigned lazily by
/// <see cref="GetClassIndex"/>, with 0 reserved for the live stream.
/// </para>
/// <para>
/// <b>Variance indicators:</b> Explained variance ratios for each principal component
/// are dispatched to the UI thread by <see cref="ReceiveInput"/> at most once per 100 ms.
/// </para>
/// <para>
...
/// </para>
/// </remarks>
public partial class SupervisedPCAViewModel : ObservableObject, ISubscriber, IDisposable
{
    #region Fields

    /// <summary>The underlying Supervised PCA model this ViewModel wraps.</summary>
    private readonly SupervisedPCA _supervisedPca;
    ...
    /// <summary>
    /// Internal ring buffer of live PC1/PC2 points shared across threads.
    ...
    /// Changing this value also trims the internal ring buffer, syncs the
    /// <see cref="Scatter3DMonitor.MaxPoints"/> limit, and marks axis bounds as dirty.
    /// </summary>
    ...
    /// <summary>
    /// Lightweight display model for a single captured PCA cluster, used to populate
    /// the legend in the UI.
    /// </summary>
    public class ClusterInfo
    {
        /// <summary>The human-readable label assigned to this cluster (e.g. <c>"rest"</c>, <c>"grip"</c>).</summary>
    ...
    /// </summary>
    public bool HasClusters => _supervisedPca?.CapturedClusters?.Count > 0;

    /// <summary>Exposes the underlying <see cref="SupervisedPCA"/> model for direct binding from the view.</summary>
    public SupervisedPCA SupervisedPCA => _supervisedPca;
    ...
    /// 3-component mode, wires up model subscriptions, schedules an initial cluster rebuild if
    /// the model already contains captured data, and registers with the shared visualisation timer.
    /// </summary>
    /// <param name="supervisedPca">The <see cref="SupervisedPCA"/> model to visualise. Must not be <see langword="null"/>.</param>
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="supervisedPca"/> is <see langword="null"/>.</exception>
    public SupervisedPCAViewModel(SupervisedPCA supervisedPca)
    {
        _supervisedPca = supervisedPca ?? throw new ArgumentNullException(nameof(supervisedPca));
        _is3D = supervisedPca.ComponentCount == 3;
        ...
        _supervisedPca.AddSubscriber(this);
        _supervisedPca.PropertyChanged += OnPcaPropertyChanged;

        if (_supervisedPca.CapturedClusters.Count > 0)
    ...
    /// <summary>
    /// Creates the default axis objects for both the scatter and variance charts.
    ...
    /// <summary>
    /// Creates the initial series for both the scatter and variance charts.
    ...
    /// <summary>
    /// Reacts to <see cref="SupervisedPCA"/> property changes.
    ...
    /// </summary>
    private void OnPcaPropertyChanged(object? sender, System.ComponentModel.PropertyChangedEventArgs e)
    {
        if (!_disposed && e.PropertyName == nameof(SupervisedPCA.CapturedClusters))
            _clusterUpdatePending = true;
    }
    ...
    /// <summary>
    /// Trims both the internal ring buffer and the UI collection to the new cap,
    /// syncs <see cref="Scatter3DMonitor.MaxPoints"/>, and marks axis bounds as dirty
    /// when <see cref="MaxScatterPoints"/> changes.
    /// </summary>
    ...
    /// pending variance or cluster-series updates. When a cluster update is processed,
    /// <see cref="RebuildFrozen3DPoints"/> is also called to keep the 3D monitor in sync;
    /// a failed 3D rebuild leaves <see cref="_clusterUpdatePending"/> set so the next tick retries it.
    /// </summary>
    private void OnTimerTick()
    {
        ...
        {
            Console.WriteLine($"[SupervisedPCAViewModel] Timer error: {ex.Message}");
        }
    }
    ...
    /// <summary>
    /// Receives a projected vector published by <see cref="SupervisedPCA"/>.
    /// </summary>
    ...
    /// by the <see cref="Scatter3DMonitor"/> with the current capture class index.
    /// </param>
    /// <remarks>
    /// Called on a background thread. PC1/PC2 are buffered in <see cref="_pointBuffer"/> under
    /// <see cref="_lock"/> and axis bounds are updated immediately. If the model is in 3D mode
    /// the full projection is forwarded to <see cref="Scatter3D"/> synchronously, tagged with
    /// class index 0 (live stream) or the current capture label's class index. Sample count and
    /// variance ratios are dispatched to the UI thread at most once per 100 ms. When a capture
    /// session is active, the cluster series rebuild is additionally throttled to once per 500 ms.
    /// </remarks>
    public void ReceiveInput(object sender, object value)
    {
        ...
        if (Is3D && projection.Count >= 3)
        {
            ...
            if (_supervisedPca.IsCapturing && !string.IsNullOrEmpty(_supervisedPca.CurrentLabel))
                classIdx = GetClassIndex(_supervisedPca.CurrentLabel);
        ...
        {
            _lastTextUpdate = now;
            var sampleCount = _supervisedPca.SampleCount;
            var varRatio = _supervisedPca.ExplainedVarianceRatio;
            Dispatcher.UIThread.Post(() =>
            {
                ...
                if (varRatio?.Count >= 2)
                {
                    ...
                    if (Is3D && varRatio.Count >= 3) Pc3Variance = varRatio[2];
                }
            }, DispatcherPriority.Background);
        }

        if (_supervisedPca.IsCapturing && (now - _lastClusterUpdate).TotalMilliseconds >= 500)
        {
    ...
    /// are taken together under <see cref="_lock"/>; neither caller holds the lock already.
    /// </remarks>
    ...
    private void StartCapture()
    {
        ...
        _supervisedPca.StartCapture(LabelInput.Trim());
    }
    ...
    private void ClearCluster(string? label)
    {
        ...
        _supervisedPca.ClearCluster(label);
        _clusterUpdatePending = true;
    }

    /// <summary>
    /// Removes all clusters from the model, resets the class-index map, clears the
    /// <see cref="Scatter3DMonitor"/> frozen points and class colours, resets axis bounds
    /// to their defaults, and schedules a cluster-series rebuild.
    /// </summary>
    [RelayCommand]
    private void ClearAllClusters()
    {
        _supervisedPca.ClearAllClusters();
        lock (_lock)
        {
            _clusterClassMap.Clear();
            _nextClassIndex = 0;
            _minX = _minY = -5; _maxX = _maxY = 5;
        }
    ...
    /// snapshot of <see cref="SupervisedPCA.CapturedClusters"/>, and syncs per-class colours
    /// from <see cref="SupervisedPCA.ClusterColors"/>. No-ops when not in 3D mode.
    /// </summary>
    /// <returns>
    ...
    private bool RebuildFrozen3DPoints()
    {
        ...
        try
        {
            ...
            var colorSnapshot = _supervisedPca.ClusterColors
            ...
            var clusterSnapshot = _supervisedPca.CapturedClusters
        ...
        {
            Console.WriteLine($"[SupervisedPCAViewModel] Frozen 3D rebuild error: {ex.Message}");
            return false;
        }
    }
    ...
    /// </summary>
    /// <remarks>
    /// Takes point-list snapshots of both <see cref="SupervisedPCA.CapturedClusters"/> and
    /// <see cref="SupervisedPCA.ClusterColors"/> before iterating to avoid collection-modified
    ...
    private void RebuildAllSeriesUi()
    {
        ...
        {
            Console.WriteLine($"[SupervisedPCAViewModel] Cluster series error: {ex.Message}");
        }
    ...
    /// <summary>
    /// Unsubscribes from the <see cref="VisualizationTimer"/> and the <see cref="SupervisedPCA"/> model,
    ...
    public void Dispose()
    {
        ...
        if (_supervisedPca != null)
        {
            _supervisedPca.RemoveSubscriber(this);
            _supervisedPca.PropertyChanged -= OnPcaPropertyChanged;
        }
        Scatter3D?.Dispose();
    }
